...
export default function Features() {
  return (
    <Section id="features">
      <h2 className="text-2xl md:text-3xl font-bold">{parseTextWithFormatting(CONFIG.features.title)}</h2>
      <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {CONFIG.features.items.map((feature) => (
          <div key={feature.title} className="card p-6 hover:glow transition">
            ...
            <p className="mt-2 text-zinc-300">{parseTextWithFormatting(feature.description)}</p>
          </div>
        ))}
      </div>
    </Section>
