...
export default function Pricing() {
  return (
    <Section id="pricing">
      <h2 className="text-2xl md:text-3xl font-bold">{CONFIG.pricing.title}</h2>
      <div className="mt-8 grid md:grid-cols-3 gap-6">
        {CONFIG.pricing.plans.map((plan) => (
          <div key={plan.name} className={`card p-6 ${plan.highlighted ? 'ring-2 ring-brand-500 glow' : ''}`}>
            ...
            <a href="#cta" className="mt-6 inline-flex w-full justify-center rounded-xl px-4 py-2 font-semibold bg-brand-500 hover:bg-brand-500/90 transition">
              {plan.ctaText}
            </a>
          </div>
        ))}
